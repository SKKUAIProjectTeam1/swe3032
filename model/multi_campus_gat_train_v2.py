...
V22-B(image_gat.py)에서 검증된 개선을 이식하고, "도로답게" 만드는 개선 3가지를 추가:
  ...
  ✓ 자동 GT corridor — 건물군을 K개 클러스터로 묶고 클러스터 대표점 간
    최단경로가 공유하는 구간을 "무조건 도로"로 자동 탐지해 BCE loss로 강제
    (사람이 라벨링하지 않음, 모든 train 캠퍼스에 자동 적용)
...
CKPT    = os.path.join(OUT_DIR, 'multi_campus_gat_v2.pth')
os.makedirs(OUT_DIR, exist_ok=True)

GT_LOSS_WEIGHT = 20.   # 자동 탐지된 "간선 corridor" 구간 BCE 가중치
...
def load_campus(img_path: str, txt_path: str) -> dict:
    ...
    return {
        ...
        'gt_edge_mask':   _auto_gt_edge_mask(is_bld, bld_nodes),
...
def loss_fn(ew: torch.Tensor, gs: torch.Tensor, c: dict, scale: float) -> torch.Tensor:
    ...
    # 6. GT road: 자동 탐지된 "간선 corridor"(여러 건물군 간 최단경로가 공유하는
    #    구간) → ew를 1로 유도. 어떤 동선을 잡아도 빠질 수 없는 길이므로
    #    MST가 놓치는 "간선도로"를 보강하는 역할
...
def plot_campus(model: MultiCampusRoadGAT, c: dict, out_path: str):
    ...
    ax.set_title(f'RWS_V6  ·  Multi-Campus GAT V23 (sparse/degree 완화)  ·  {title}',
...
def visualize_all(model: MultiCampusRoadGAT, campuses: list[dict], out_dir: str = OUT_DIR):
    ...
    os.makedirs(out_dir, exist_ok=True)
    for c in campuses:
        plot_campus(model, c, os.path.join(out_dir, f'RWS_V6_road_{c["name"]}.png'))
    ...
    if args.infer:
        ...
        if not hits:
            ...
            plot_campus(model, hits[0],
                        os.path.join(OUT_DIR, f'RWS_V6_road_{hits[0]["name"]}.png'))
    else:
