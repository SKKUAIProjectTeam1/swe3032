"""
multi_campus_gat_train_v4.py  —  Multi-Campus GAT Road Designer (V23 + OSM GT + 지름길)

V3(OSM GT)를 베이스로, "우회 방지" 지름길 GT를 추가:
  ✓ _shortcut_gt_edge_mask — 서로 멀리 떨어진 건물군 쌍(군집 쌍 거리 상위 50%)
    사이의 최단경로를 임계값 없이 GT로 포함. _auto_gt_edge_mask/OSM GT만으로는
    "한 쌍만 지나는 먼 지름길"이 빠질 수 있는데, 이게 없으면 양 끝을 오갈 때
    항상 크게 우회해야 하므로 비효율적 — 모든 train 캠퍼스에 추가 적용

train/test split 유지: TEST_CAMPUSES 5개는 학습에서 제외(hold-out).

실행:
  python model/multi_campus_gat_train_v4.py                      # 학습 → hold-out 시각화
  python model/multi_campus_gat_train_v4.py --epochs 20          # epoch 수 조절 (시험 실행)
  python model/multi_campus_gat_train_v4.py --load               # 저장 모델 로드 → 시각화
  python model/multi_campus_gat_train_v4.py --load --infer sungkyunkwan_university
"""
...
CKPT     = os.path.join(OUT_DIR, 'multi_campus_gat_v4.pth')
...
def plot_campus(model: MultiCampusRoadGAT, c: dict, out_path: str):
    ...
    ax.set_title(f'RWS_V8  ·  Multi-Campus GAT V23 (OSM GT + 지름길)  ·  {title}',
...
def visualize_all(model: MultiCampusRoadGAT, campuses: list[dict], out_dir: str = OUT_DIR):
    ...
    os.makedirs(out_dir, exist_ok=True)
    for c in campuses:
        plot_campus(model, c, os.path.join(out_dir, f'RWS_V8_road_{c["name"]}.png'))
    ...
    if args.infer:
        ...
        if not hits:
            ...
            plot_campus(model, hits[0],
                        os.path.join(OUT_DIR, f'RWS_V8_road_{hits[0]["name"]}.png'))
    else:
