"""
multi_campus_gat_train_v6.py  —  Multi-Campus GAT Road Designer (V23 + OSM GT + 지름길 + 건물별 가지)

V5(sparse/degree 완화)는 kangwon 등 hold-out에서 효과가 거의 없었음
(여전히 "지렁이"형 단일 경로). 원인: _auto_gt_edge_mask/_osm_gt_edge_mask/
_shortcut_gt_edge_mask 모두 "군집/캠퍼스 단위 동선"만 GT로 잡고, 개별
건물에 직접 닿는 가지(spur)는 GT에 없음 — 모델이 "건물 근처만 지나가면
된다"고 학습해 분기를 만들 동기가 없었음.

  ✓ _spur_gt_edge_mask — 각 건물의 최근접 free node에서 기존 GT 도로까지의
    최단경로를 추가 GT로 표시. 모든 캠퍼스(hold-out 포함)에 적용되므로
    kangwon 같은 hold-out에도 직접 효과.

train/test split 유지: TEST_CAMPUSES 5개는 학습에서 제외(hold-out).

실행:
  python model/multi_campus_gat_train_v6.py                      # 학습 → hold-out 시각화
  python model/multi_campus_gat_train_v6.py --epochs 20          # epoch 수 조절 (시험 실행)
  python model/multi_campus_gat_train_v6.py --load               # 저장 모델 로드 → 시각화
  python model/multi_campus_gat_train_v6.py --load --infer sungkyunkwan_university
"""
...
CKPT     = os.path.join(OUT_DIR, 'multi_campus_gat_v6.pth')
...
def _spur_gt_edge_mask(is_bld: np.ndarray, building_nodes: list[int],
                        gt_mask: torch.Tensor | None) -> torch.Tensor | None:
    """각 건물에서 기존 GT 도로까지의 최단경로를 "가지(spur)" GT로 추가.

    _auto_gt_edge_mask/_osm_gt_edge_mask/_shortcut_gt_edge_mask는 모두
    군집/캠퍼스 단위 동선만 GT로 표시하므로, 그 경로에서 떨어진 개별
    건물에는 GT가 전혀 없다 — 모델이 "건물 근처만 지나가면 된다"고
    학습해 건물별 가지가 생기지 않음. building_nodes(건물별 최근접
    free node) 각각에서 기존 GT 도로까지의 최단경로를 추가 GT로 표시해
    건물마다 직접 닿는 가지를 supervise한다.
    GT가 전혀 없으면(gt_mask=None) None.
    """
    if gt_mask is None or len(building_nodes) == 0:
        return None
    ...
    gt_road = np.zeros(N, dtype=bool)
    gt_road[src_all[gt_mask_np]] = True
    gt_road[dst_all[gt_mask_np]] = True
    gt_road &= free
    gt_idx = np.where(gt_road)[0]
    if len(gt_idx) == 0:
        return None

    dmat, pred = dijkstra(graph, indices=building_nodes, return_predecessors=True, unweighted=True)

    spur = np.zeros(N, dtype=bool)
    for i, b in enumerate(building_nodes):
        d_to_gt = dmat[i, gt_idx]
        if not np.isfinite(d_to_gt).any():
            continue
        target = gt_idx[np.argmin(d_to_gt)]
        node = target
        while node != b and node >= 0:
            spur[node] = True
            node = pred[i, node]
        spur[b] = True
    spur &= free

    mask = spur[src_all] & spur[dst_all]
    if not mask.any():
        return None
    return torch.tensor(mask, dtype=torch.bool).to(DEVICE)
...
def load_campus(img_path: str, txt_path: str) -> dict:
    ...
    # 건물별 가지(spur) GT: 각 건물에서 기존 GT 도로까지의 최단경로 추가
    spur_mask = _spur_gt_edge_mask(is_bld, bld_nodes, gt_mask)
    if spur_mask is not None:
        gt_mask = spur_mask if gt_mask is None else (gt_mask | spur_mask)
        gt_src += '+spur'
...
def plot_campus(model: MultiCampusRoadGAT, c: dict, out_path: str):
    ...
    ax.set_title(f'RWS_V10  ·  Multi-Campus GAT V23 (OSM GT + 지름길 + 건물별 가지)  ·  {title}',
...
def visualize_all(model: MultiCampusRoadGAT, campuses: list[dict], out_dir: str = OUT_DIR):
    ...
    os.makedirs(out_dir, exist_ok=True)
    for c in campuses:
        plot_campus(model, c, os.path.join(out_dir, f'RWS_V10_road_{c["name"]}.png'))
    ...
    if args.infer:
        ...
        if not hits:
            ...
            plot_campus(model, hits[0],
                        os.path.join(OUT_DIR, f'RWS_V10_road_{hits[0]["name"]}.png'))
    else:
