...
# ✅ self-loop 추가: attention normalize 보장
...
class SparseGATLayer(nn.Module):
    ...
    def __init__(self, in_dim, out_dim):
        ...
        self.norm = nn.LayerNorm(out_dim)  # ✅ 발산 방지
    ...
    def forward(self, x, edge_index):
        ...
        return self.norm(F.elu(out)), alpha
...
class CivilEngineerGAT(nn.Module):
    def __init__(self, res):
        ...
        # ✅ 모든 Linear 가중치 작게 초기화
        for m in self.edge_predictor:
...
for step in range(50001):
    ...
    # ✅ 모든 loss warmup: 초반엔 loss 전체를 낮게 시작
    scale = min(1.0, step / WARMUP_STEPS)

    collision_mask = (is_building_tensor[src] > 0) | (is_building_tensor[dst] > 0)
    collision_loss = torch.sum(edge_weights[collision_mask] * scale * 10000.0)

    ridge_score = (ridge_tensor[src] + ridge_tensor[dst]) / 2.0
    ridge_loss = torch.sum(edge_weights * (1.0 - ridge_score)) * (scale * 9.9 + 0.1)  # 0.1 → 10.0
    ...
    conn_loss = torch.sum(torch.pow(F.relu(2.0 - node_strength[building_nodes_t]), 2)) * (scale * 29.0 + 1.0)  # 1 → 30
    ...
    if step % 5000 == 0:
        ...
        print(f"  Step {step:5d} | Loss: {total_loss.item():.2f} | Connected: {conn_count}/23 | Collisions: {collision_loss.item()/max(scale*10000,1):.4f} | LR: {lr_now:.5f}")
