"""
multi_campus_gat_train_v3.py  —  Multi-Campus GAT Road Designer (V23 + OSM GT)

V2(sparse/degree 완화)를 베이스로, GT 신호를 휴리스틱 corridor 대신
실제 도로 데이터로 교체:
  ✓ OSM road mask GT — collegemap/road_masks/*.npy (osm_road_crawler.py로
    수집한 실제 도로망 래스터)를 GT 엣지 마스크로 사용. 휴리스틱 corridor
    (~400칸)보다 훨씬 풍부한 ~2000-3000칸의 실제 도로 패턴을 BCE로 학습
  ✓ 마스크가 없는 캠퍼스(49/86개만 보유)는 기존 자동 GT corridor로 폴백

train/test split 유지: TEST_CAMPUSES 5개는 학습에서 제외(hold-out).
kangwon, hanyang_erica는 OSM 마스크가 있지만 hold-out이므로 학습에 쓰이지
않고, 생성 결과를 실제 도로와 비교 평가하는 용도로만 활용 가능.

실행:
  python model/multi_campus_gat_train_v3.py                      # 학습 → hold-out 시각화
  python model/multi_campus_gat_train_v3.py --epochs 20          # epoch 수 조절 (시험 실행)
  python model/multi_campus_gat_train_v3.py --load               # 저장 모델 로드 → 시각화
  python model/multi_campus_gat_train_v3.py --load --infer sungkyunkwan_university
"""
...
CKPT     = os.path.join(OUT_DIR, 'multi_campus_gat_v3.pth')
os.makedirs(OUT_DIR, exist_ok=True)

GT_LOSS_WEIGHT = 20.   # GT 도로(OSM 마스크 또는 자동 corridor) 구간 BCE 가중치
SPARSE_WEIGHT  = 8.    # 총 도로량 억제 강도 (V23: 15 → 완화, 더 밀도 있는 네트워크 허용)
DEGREE_WEIGHT  = 1.5   # 교차점(분기) 억제 강도 (V23: 3 → 완화)
DEGREE_THRESH  = 5.    # 패널티 없이 허용하는 교차점 차수 (V23: 4)
...
def plot_campus(model: MultiCampusRoadGAT, c: dict, out_path: str):
    ...
    ax.set_title(f'RWS_V7  ·  Multi-Campus GAT V23 (OSM GT)  ·  {title}',
...
def visualize_all(model: MultiCampusRoadGAT, campuses: list[dict], out_dir: str = OUT_DIR):
    ...
    os.makedirs(out_dir, exist_ok=True)
    for c in campuses:
        plot_campus(model, c, os.path.join(out_dir, f'RWS_V7_road_{c["name"]}.png'))
    ...
    if args.infer:
        ...
        if not hits:
            ...
            plot_campus(model, hits[0],
                        os.path.join(OUT_DIR, f'RWS_V7_road_{hits[0]["name"]}.png'))
    else:
